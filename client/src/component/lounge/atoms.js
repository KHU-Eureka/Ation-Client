...
import {
  ...
  LEADERNAME_STYLE,
  LEADERIMG_STYLE,
  VALIDBTN_STYLE_ENTER,
  INVALIDBTN_STYLE_ENTER,
  BTN_HOVERSTYLE,
  BTN_HOVEROUTSTYLE,
  BTN_CLICKSTYLE,
  BTN_CLICKOUTSTYLE,
  MODULE_HOVEROUTSTYLE,
  MODULE_HOVERSTYLE,
} from "./atomStyleSheet";
...
const senseImg = (sense, mode) => {
  switch (sense) {
    case "눈":
      return <img className="senseImg" src={mode === 1 ? w_eye : o_eye} />;
    case "코":
      return <img className="senseImg" src={mode === 1 ? w_nose : o_nose} />;
    case "입":
      return <img className="senseImg" src={mode === 1 ? w_ear : o_ear} />;
    case "손":
      return <img className="senseImg" src={mode === 1 ? w_mouth : o_mouth} />;
    case "귀":
      return <img className="senseImg" src={mode === 1 ? w_hand : o_hand} />;
  }
  return null;
};
...
const tag = (mainCategory, subCategoryList) => {
  return (
    <div className="tag-container">
      <span className="tag">{`#${mainCategory}`}</span>
...
const enterBtn = (isValid, loungeId, personaId) => {
      ...
      if (target.className !== "pin") {
        ...
        try {
          const response = await axios.put(
            `${process.env.REACT_APP_SERVER_HOST}/api/lounge/${loungeId}/enter/${personaId}`,
            {},
            {
              headers: {
  ...
  return (
    <button
      className={isValid ? "valid btn" : "invalid"}
      onClick={isValid ? clickStyleHandler : null}
      style={isValid ? VALIDBTN_STYLE_ENTER : INVALIDBTN_STYLE_ENTER}
      onMouseOver={hoverStyleHandler}
      onMouseOut={hoverOutStyleHandler}
    >
      입장
    </button>
  );
...
export const deleteBtn = (url, id, setDeleteLounge) => {
  return (
    <img
      ...
      onClick={() =>
        deleteHandler(url, id).then((data) => setDeleteLounge(data.data))
      }
    />
  );
};
...
export const ImgBox = ({ obj, isPin }) => {
  ...
  return (
    <div
      className="imgbox-container"
      style={{
        position: "relative",
        cursor: "pointer",
        background: "transparent",
      }}
      onClick={({ target }) => enterLounge(target, obj.id, activePersonaId)}
    >
      <div
        className="imgbox-container img"
        style={{ position: "absolute", zIndex: "-1", top: "0px" }}
      >
        <LazyLoadImage alt="..." effect="blur" src={obj.imgPath} />
      </div>
      <div className="imgbox-wrap-container">
        <div className="header-container">{senseImg(obj.sense.name, 1)}</div>
      ...
      {isPin ? <LoungePinup loungeId={obj.id} /> : null}
    </div>
  );
};
...
export const Modulebox = ({ obj, isSense, link, setDeleteLounge }) => {
  ...
  return (
    <div
      className="modulebox-container"
      onMouseOver={!isSense ? hoverStyleHandler : null}
      onMouseOut={!isSense ? hoverOutStyleHandler : null}
    >
      <div className="modulebox-wrap-container">
        {isSense ? (
          <div className="left-container">{senseImg(obj.sense.name, 2)}</div>
        ) : null}
